...
const fetchBecaUser = async () => {
    try {
        ...
        // ultimoPeriodo.value = periodos.value[0];

        // if (beca.value.ID_PERIODO === ultimoPeriodo.value.ID_PERIODO) {
        //     console.log("Activa");
        // } else {
...
<template>
    ...
    <div v-if="beca && beca.length > 0">
        <Card class="transform transition-all duration-300 hover:shadow-xl hover:-translate-y-1 w-full md:w-96">
            <template #header>
                <div class="relative">
                    <div class="absolute top-0 right-0 m-2">
                        <Tag v-if="beca[0].ID_ESTADO == 7" icon="pi pi-check" value="Activa" severity="success"
                            rounded />
                        <Tag v-if="beca[0].ID_ESTADO == 8" icon="pi pi-ban" value="Inactiva" severity="warn" rounded />
                        <Tag v-if="beca[0].ID_ESTADO == 6" icon="pi pi-times" value="Finalizada" severity="info"
                            rounded />
            ...
            <template #title>
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-xl font-semibold">{{ beca[0].CEDULA_ESTUDIANTE }}</span>
                </div>
            </template>
            ...
            <template #content>
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-medium">{{ beca[0].TIPO_BECA }}</span>
                    </div>

                    <!-- Porcentaje -->
                    <div class="flex items-center gap-2">
                        <i class="pi pi-percentage text-green-500"></i>
                        <ProgressBar :value="beca[0].PORCENTAJE" :showValue="true" class="w-full" />
                    </div>
                    ...
                    <div class="flex flex-col gap-2">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p class="font-medium">{{ beca[0].NOMBRE_PERIODO }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p class="font-medium">{{ beca[0].PERIODO_CADUCIDAD }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                <Message v-if="beca[0].ID_ESTADO == 8 && beca[0].RENOVACION === null" class="flex justify-center"
                    style="margin-top: 10px; cursor: pointer;" severity="error" variant="simple" @click="renovacion()">
                    SE
                    NECESITA RENOVACION</Message>
            </template>
